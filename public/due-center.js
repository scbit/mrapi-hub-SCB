...
const S={token:localStorage.getItem(TK)||'',user:null,meta:null,templates:[],rows:[],groups:{},selected:new Set(),reads:0,mode:'vencidos'};
...
function shell(){
  ...
  <header class="dueTop"><a class="dueBrand" href="/"><img src="/assets/tenant-logo"><div><b>MR API HUB</b><small>${esc(window.MRAPI_TENANT?.shortName||'MRAPI')}</small></div></a>
    ...
    <span class="spacer"></span><span class="version-pill">v1.5.25</span><span class="who">${esc(S.user?.name||S.user?.email||'')}</span>
  </header>
  <main class="dueMain">
    ...
    <section class="sendCard">
      <div class="sendHead"><div><b id="selectedPill">0 seleccionados</b><span>Enviar seguimiento con plantilla aprobada</span></div><button class="btn secondary small" id="clearSel">Limpiar selección</button></div>
      <div class="sendGrid">
        ...
        <button class="btn sendBtn" id="sendBtn">Enviar a seleccionados</button>
      </div>
  ...
  $('sendBtn').onclick=sendSelected;$('modalClose').onclick=closeModal;$('modalBack').onclick=e=>{if(e.target===$('modalBack'))closeModal()};
}
...
async function start(){if(!S.token)return location.href='/crm';shell();try{S.meta=await api('/api/due-center/meta');fillMeta();await Promise.all([loadTemplates(),load()])}catch(e){notice(e.message,'error')}}
start();
})();