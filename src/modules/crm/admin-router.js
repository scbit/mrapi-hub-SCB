...
function periodRange(period,start,end){const p=String(period||"today").toLowerCase(),today=todayBA();if(p==="total")return {period:p,start:"",end:"",startDate:null,endDate:null};if(p==="custom"&&/^\d{4}-\d{2}-\d{2}$/.test(start)&&/^\d{4}-\d{2}-\d{2}$/.test(end)){const a=start<=end?start:end,b=start<=end?end:start;return {period:p,start:a,end:b,startDate:new Date(`${a}T00:00:00Z`),endDate:new Date(`${addDays(b,1)}T00:00:00Z`)};}let s=today,e=today;if(p==="yesterday"){s=addDays(today,-1);e=s;}else if(p==="week"){const d=new Date(`${today}T12:00:00Z`),day=d.getUTCDay()||7;s=addDays(today,1-day);}else if(p==="last_week"){const d=new Date(`${today}T12:00:00Z`),day=d.getUTCDay()||7;const thisMonday=addDays(today,1-day);s=addDays(thisMonday,-7);e=addDays(thisMonday,-1);}else if(p==="month")s=today.slice(0,8)+"01";const valid=["today","yesterday","week","last_week","month"];const normalized=valid.includes(p)?p:"today";return {period:normalized,start:s,end:e,startDate:new Date(`${s}T00:00:00Z`),endDate:new Date(`${addDays(e,1)}T00:00:00Z`)};}
