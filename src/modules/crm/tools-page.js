...
router.get("/vencimientos",(req,res)=>{
  const t=getTenant();
  const html=`<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="theme-color" content="${t.branding.primaryDark}"><title>Centro de Vencimientos · ${t.branding.shortName}</title><link rel="icon" href="/assets/favicon.svg" type="image/svg+xml"><link rel="stylesheet" href="/assets/due-center.css?v=1.5.26"><style>${themeCss(t)}</style></head><body><script>window.MRAPI_TENANT=${JSON.stringify(browserTenant(t))};</script><script src="/assets/due-center.js?v=1.5.26"></script></body></html>`;
